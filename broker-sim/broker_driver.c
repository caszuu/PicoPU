...
void save_fb() {
  ...
  for (uint16_t y = 0; y < fb_extent[1] / RENDER_QUAD_SIZE; y++) {
    for (uint16_t x = 0; x < fb_extent[0] / RENDER_QUAD_SIZE; x++) {
      for (uint8_t tile_i = 0; tile_i < RENDER_QUAD_SIZE * 2; tile_i++) {
        uint32_t lin_i = (x * 2) + (tile_i % 2) /* x axis */ +
                         ((y * 2 + tile_i / 2) * fb_extent[0]) /* y axis */;
        uint32_t fb_i = (x * 4) /* x axis */ +
                        (y * 2 * fb_extent[0]) /* y axis*/ +
                        tile_i /* tile index */;
        ...
        depth_buf[lin_i] =
            fb_depth_buffer[fb_i] / (32 / 8); // map to 8-bit channel range
      }
    }
  }
  ...
  stbi_write_png("broker_sim_color0.png", fb_extent[0], fb_extent[1], 4,
                 color_buf, sizeof(struct rgba_color) * fb_extent[0]);
  stbi_write_png("broker_sim_depth.png", fb_extent[0], fb_extent[1], 1,
                 depth_buf, sizeof(uint8_t) * fb_extent[0]);
...
void setup_serial(int speed, int parity) {
  ...
  if (picopu_fd == -1) {
    printf("can't access picopu device on %s: %s", picopu_device,
           strerror(errno));
    exit(-1);
  }
...
bool recv_shader(void **res, gcs_type_t expected_type,
                 gcs_type_t secondary_expected_type) {
  uint16_t size;
  ...
  if (*(gcs_type_t *)(buf) != expected_type &&
      *(gcs_type_t *)(buf) != secondary_expected_type) {
...
int main() {
  ...
  fb_color_buffer =
      malloc(sizeof(struct rgba_color) * fb_extent[0] * fb_extent[1]);
  ...
  struct gcs_vs_header *v_stream =
      malloc(sizeof(struct gcs_vs_header) + sizeof(vertex_buffer));

  v_stream->type = gcs_type_vs;
  v_stream->primitive_count = 1;
  v_stream->base_vertex = 0;

  memcpy((void *)v_stream + sizeof(struct gcs_vs_header), vertex_buffer,
  ...
  for (uint8_t i = 0; i < prim_output->primitive_count; i++) {
    ...
    struct gcs_fs_header *f_stream =
        malloc(sizeof(struct gcs_fs_header) + sizeof(struct clip_point) * 3 +
               vertex_output_stride * 3);
    f_stream->type = gcs_type_fs;

    f_stream->line_offset = prim_output->shading_area[1];
    f_stream->line_count =
        prim_output->shading_area[3] - prim_output->shading_area[1];
    ...
    memcpy(fs_seek, &vertex_output_buf[i * vertex_output_stride * 3],
           vertex_output_stride * 3);
    fs_seek += vertex_output_stride * 3 * CHIPS_PER_CLUSTER;

    stream_shader(f_stream, sizeof(struct gcs_fs_header) +
                                sizeof(struct clip_point) * 3 +
                                vertex_output_stride * 3);
    free(f_stream);
    ...
    while (true) {
      ...
      if (f_output->type == gcs_type_ready)
        break;
      ...
      assert(f_output->fb_index_base <
             fb_extent[0] * fb_extent[1]); // protect against writting outside
                                           // fb by a faulty index_base

      for (uint8_t i = 0; i < f_output->tile_count; i++) {
        // late depth-test

        depth_t *fb_dt = &fb_depth_buffer[f_output->fb_index_base +
                                          i * RENDER_QUAD_SIZE * 2];
        const struct depth_tile *dt = &dt_buf[i];

        uint8_t depth_mask =
            (fb_dt[0] >= dt->d[0]) << 0 | (fb_dt[1] >= dt->d[1]) << 1 |
            (fb_dt[2] >= dt->d[2]) << 2 | (fb_dt[3] >= dt->d[3]) << 3;

        depth_mask &= cv_mask_buf[i / 2] >>
                      (4 * (i % 2)); // only here to test if depth test should
                                     // be written back to fb

        memcpy(&fb_dt[0], (depth_mask & (1 << 0)) ? &dt->d[0] : &fb_dt[0],
               sizeof(depth_t));
        memcpy(&fb_dt[1], (depth_mask & (1 << 1)) ? &dt->d[1] : &fb_dt[1],
               sizeof(depth_t));
        memcpy(&fb_dt[2], (depth_mask & (1 << 2)) ? &dt->d[2] : &fb_dt[2],
               sizeof(depth_t));
        memcpy(&fb_dt[3], (depth_mask & (1 << 3)) ? &dt->d[3] : &fb_dt[3],
               sizeof(depth_t));

        // FIXME: this is not an ideal impl (probably just depth-test both 4-bit
        // masks at once)
        uint8_t cv_bit_offset = 4 * (i % 2);
        cv_mask_buf[i / 2] &=
            depth_mask << cv_bit_offset | 0xF0 >> cv_bit_offset;
      }
      ...
      for (uint8_t i = 0; i < f_output->tile_count; i++) {
        ...
        memcpy(&fb_ct[0], (cv_mask & (1 << 0)) ? &ct->c[0] : &fb_ct[0],
               sizeof(struct rgba_color));
        memcpy(&fb_ct[1], (cv_mask & (1 << 1)) ? &ct->c[1] : &fb_ct[1],
               sizeof(struct rgba_color));
        memcpy(&fb_ct[2], (cv_mask & (1 << 2)) ? &ct->c[2] : &fb_ct[2],
               sizeof(struct rgba_color));
        memcpy(&fb_ct[3], (cv_mask & (1 << 3)) ? &ct->c[3] : &fb_ct[3],
               sizeof(struct rgba_color));
      }
    };
