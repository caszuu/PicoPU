...
/* static void rast_request_tile(struct tile_slot *slot) {
    bool z_clear = ((struct gcs_cbuf_state *)(chip_state.cbuf))->clear_bits & CLEAR_DEPTH;
    bool c_clear = ((struct gcs_cbuf_state *)(chip_state.cbuf))->clear_bits & CLEAR_COLOR;

    uint16_t fetch_tile_idx = ;

    struct gcs_fetch_header fetch_req = {
        .type = gcs_type_fetch,
        .req_count = 0,
    };
    struct gcs_fetch fetch_buf[2];

    // attachment tile clears / fetch reqs

    if (z_clear) {
        dma_memset32(RAST_DMACH0, slot->z_tile, -1.f, sizeof(slot->z_tile) / sizeof(uint32_t));
    } else {
        fetch_buf[fetch_req.req_count++] = (struct gcs_fetch){
            .tile_idx = fetch_tile_idx,
            .attach_idx = 0,
        };
    }

    if (c_clear && blending_enabled) {
        dma_memset32(RAST_DMACH1, slot->c_tile, cbuf->clear_vals[1], sizeof(slot->c_tile) / sizeof(uint32_t));
    } else {
        fetch_buf[fetch_req.req_count++] = (struct gcs_fetch){
            .tile_idx = fetch_tile_idx,
            .attach_idx = 1,
    ...
    // request tle fetches, if any

    if (z_clear && c_clear)
        return;

    scs_tx_packet(&fetch_req, sizeof(struct gcs_fetch_header) + sizeof(struct gcs_fetch) * fetch_req.req_count);
    scs_queue_rx(gcs_type_fetch_data, &rx_fetch_buf, slot->rx_fetch);
} */
