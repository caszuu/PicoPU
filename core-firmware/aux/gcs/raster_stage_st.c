...
struct trig_rast_state {
    // active trig states
    struct trig_state trig_states[MAX_TRIGS_PER_BATCH];
};
...
static inline void rast_setup_trigs() {
    ...
    for (uint32_t i = 0; i < v2f.prim_count; i++) {
        ...
        int32_t v1v2_c = init_trig_edge(trig, 0, p1, p2, origin);
        init_trig_edge(trig, 1, p2, p0, origin);
        init_trig_edge(trig, 2, p0, p1, origin);
...
static inline bool dispatch_frag(int32_t w[], float wf[], float z, int32_t area, float area_inv, v2i32 p, int32_t tile_x, int32_t tile_y) {
    ...
    if (z_tile[tile_x + tile_y * RASTER_TILE_SIZE] >= z)
        return false;
            ...
            if (tile_fetched && cv_tile) {
                ...
                for (uint32_t x = 0; x < 4; x++) {
                    for (uint32_t y = 0; y < 4; y++) {
                        if ((cv_tile >> (x + y * 4)) & 1)
                            dvi_fb[p[0] + x + (p[1] + y) * 640] = colour_rgb332(c_tile[x + y * 4]);
                    }
                }
            }
        }
