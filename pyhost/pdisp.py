...
def submit_gcs_batch(cmd: tuple) -> None:
    ...
    v_streams = [v_data[i:i + 6] for i in range(0, len(v_data), 6)]
...
def cmd_ld_img(args: list[str]) -> None:
    ...
    cmd_data = SIProto.pack_ld_cbuf_inline(64, img_data)
...
def ld_cube_model(args: list[str]) -> None:
    gen_cube_model(args[0])
